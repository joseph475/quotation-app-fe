 ...
 * @param {Object} props.data - Purchase report data
 * @param {Object} props.dateRange - Date range for the report
 */
const PurchaseReportComponent = ({ data, dateRange }) => {
  ...
  useEffect(() => {
    ...
    let grouped = {};
    let totalPurchases = data.totalSpent || 0; // Use totalSpent from data if available
    let totalItems = 0;
    ...
    data.purchases.forEach(purchase => {
      ...
      grouped[groupKey].total += purchase.totalAmount || purchase.total || 0; // Use totalAmount if available, fall back to total
      
      // Count items and track supplier purchases for top suppliers
      ...
      const supplierName = typeof purchase.supplier === 'string' 
        ? purchase.supplier 
      ...
      supplierPurchases[supplierName].total += purchase.totalAmount || purchase.total || 0; // Use totalAmount if available, fall back to total
      
      // Don't accumulate totalPurchases here as we're using data.totalSpent
    });
    ...
    // Use averagePurchase from data if available, otherwise calculate it
    const averagePurchase = data.averagePurchase || (data.purchases.length > 0 ? totalPurchases / data.purchases.length : 0);
  ...
  return (
    <div>
      ...
      <div class="mb-6 print:mb-4">
        <h2 class="text-xl font-bold text-gray-900 print:text-lg">Purchase Report</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 print:grid-cols-3 print:gap-2 print:mb-4">
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
          ...
          <p class="text-2xl font-bold text-gray-900 print:text-xl">{formatCurrency(summary.totalPurchases)}</p>
        </div>
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
          <h3 class="text-sm font-medium text-gray-500">Total Items Purchased</h3>
          <p class="text-2xl font-bold text-gray-900 print:text-xl">{summary.totalItems}</p>
        </div>
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
          <h3 class="text-sm font-medium text-gray-500">Average Purchase Value</h3>
          <p class="text-2xl font-bold text-gray-900 print:text-xl">{formatCurrency(summary.averagePurchase)}</p>
        </div>
      </div>
          ...
          <thead class="bg-gray-50 print:bg-gray-100">
            <tr>
              ...
              >
                ...
                <span class="hidden print:inline">Purchase Count</span>
              </th>
              <th 
                scope="col" 
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
              ...
              >
                ...
                <span class="hidden print:inline">Items Purchased</span>
              </th>
              <th 
                scope="col" 
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
              ...
              >
                ...
                <span class="hidden print:inline">Total Amount</span>
              </th>
            </tr>
          </thead>
      ...
      {/* Top Suppliers */}
      <div class="mb-6 print:mb-4">
        <h3 class="text-lg font-medium text-gray-900 mb-3 print:text-base">Top Suppliers</h3>
