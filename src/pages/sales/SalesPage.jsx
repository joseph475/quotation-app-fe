    ...
    <div>
      ...
      <div class="bg-white shadow rounded-lg mb-6">
        <div class="p-4 sm:p-6">
          {/* Mobile-first layout */}
          <div class="space-y-4 lg:space-y-0 lg:flex lg:justify-between lg:items-center">
            {/* Search */}
            <div class="relative flex-1 lg:max-w-md">
              ...
              <input
                ...
                class="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 text-sm"
              />
            </div>

            {/* Filters and Actions */}
            <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 lg:ml-4">
              {/* Date Filter and Refresh Button in one row */}
              <div class="flex justify-between">
                ...
                <div class="w-32 sm:w-36">
                  <select
                    ...
                    class="block w-full pl-3 pr-8 py-2.5 text-sm border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 rounded-md"
                  >
                ...
                {/* Refresh Button */}
                <button 
                  class="btn btn-outline flex items-center justify-center px-3 py-2"
                  onClick={() => fetchSales(true)}
                  disabled={loading}
                >
                  <svg class={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd" />
                  </svg>
                  <span class="hidden sm:inline ml-2">{loading ? 'Refreshing...' : 'Refresh'}</span>
                </button>
              </div>
      ...
      <div class="border-b border-gray-200 mb-6">
        <nav class="-mb-px flex overflow-x-auto scrollbar-hide">
          <button
            ...
            class={`flex-shrink-0 whitespace-nowrap py-4 px-3 sm:px-4 border-b-2 font-medium text-sm ${
              activeTab === 'all'
          ...
          >
            <span class="hidden sm:inline">Partially Paid</span>
            <span class="sm:hidden">Partial</span>
          </button>
          <button
        ...
        {filteredSales.length === 0 ? (
          <div class="bg-white shadow rounded-lg p-6 text-center">
            <div class="flex flex-col items-center">
              ...
              <p class="text-xs text-gray-400 mt-1">Try adjusting your filters or create a new sale</p>
            </div>
          </div>
        ) : (
          filteredSales.map((sale) => (
            <div key={sale.id} class="bg-white shadow rounded-lg p-4 border-l-4 border-l-primary-500">
              {/* Header */}
              <div class="flex justify-between items-start mb-3">
                <div>
                  <h3 class="text-sm font-medium text-primary-600">
                    {sale.sale_number || sale.saleNumber}
                  </h3>
                ...
                <span class={`px-2 py-1 text-xs font-semibold rounded-full ${
                  ...
                  {(sale.payment_status || sale.status) === 'paid' ? 'Paid' : 
                   (sale.payment_status || sale.status) === 'partially_paid' ? 'Partial' :
                   (sale.payment_status || sale.status) === 'pending' ? 'Pending' :
                   (sale.payment_status || sale.status) === 'cancelled' ? 'Cancelled' :
                   (sale.payment_status || sale.status) === 'refunded' ? 'Refunded' : 
                   (sale.payment_status || sale.status)}
      ...
      <Modal
        ...
        footer={
          selectedSale && (
            <div className="flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0 w-full">
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm sm:text-lg font-bold text-primary-600">₱{(selectedSale?.total || 0).toFixed(2)}</p>
                </div>
              </div>
              ...
              <div className="flex space-x-3 w-full sm:w-auto">
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-colors duration-200"
                  disabled={formLoading}
                >
                  <svg className="h-4 w-4 mr-2 inline text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                ...
                <button
                  ...
                  form="edit-sale-form"
                  disabled={formLoading}
                  className="w-full sm:w-auto px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                >
                  {formLoading ? (
                    <>
                    ...
                    <>
                      <svg className="h-4 w-4 mr-2 inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
